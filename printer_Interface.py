# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'printers.ui'
#
# Created by: PyQt5 UI code generator 5.15.10
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_Dialog(object):
    def setupUi(self, Dialog):
        Dialog.setObjectName("Dialog")
        Dialog.resize(480, 620)
        Dialog.setStyleSheet("background-color: black;\n"
"color: white;")
        self.verticalLayout = QtWidgets.QVBoxLayout(Dialog)
        self.verticalLayout.setObjectName("verticalLayout")
        self.label = QtWidgets.QLabel(Dialog)
        self.label.setStyleSheet("color : white;\n"
"font-size : 28px;")
        self.label.setObjectName("label")
        self.verticalLayout.addWidget(self.label)
        self.macAddress = QtWidgets.QLabel(Dialog)
        self.macAddress.setStyleSheet("color:white;")
        self.macAddress.setObjectName("macAddress")
        self.verticalLayout.addWidget(self.macAddress)
        self.macAddressLine = QtWidgets.QLineEdit(Dialog)
        self.macAddressLine.setStyleSheet("color:white;")
        self.macAddressLine.setObjectName("macAddressLine")
        self.verticalLayout.addWidget(self.macAddressLine)
        self.label_3 = QtWidgets.QLabel(Dialog)
        self.label_3.setStyleSheet("color:white;")
        self.label_3.setObjectName("label_3")
        self.verticalLayout.addWidget(self.label_3)
        self.brand_line = QtWidgets.QLineEdit(Dialog)
        self.brand_line.setObjectName("brand_line")
        self.verticalLayout.addWidget(self.brand_line)
        self.label_4 = QtWidgets.QLabel(Dialog)
        self.label_4.setStyleSheet("color:white;")
        self.label_4.setObjectName("label_4")
        self.verticalLayout.addWidget(self.label_4)
        self.model_line = QtWidgets.QLineEdit(Dialog)
        self.model_line.setObjectName("model_line")
        self.verticalLayout.addWidget(self.model_line)
        self.label_5 = QtWidgets.QLabel(Dialog)
        self.label_5.setStyleSheet("color:white;")
        self.label_5.setObjectName("label_5")
        self.verticalLayout.addWidget(self.label_5)
        self.ink_storage_line = QtWidgets.QLineEdit(Dialog)
        self.ink_storage_line.setObjectName("ink_storage_line")
        self.verticalLayout.addWidget(self.ink_storage_line)
        self.label_6 = QtWidgets.QLabel(Dialog)
        self.label_6.setStyleSheet("color:white;")
        self.label_6.setObjectName("label_6")
        self.verticalLayout.addWidget(self.label_6)
        self.arrival_date_line_2 = QtWidgets.QLineEdit(Dialog)
        self.arrival_date_line_2.setObjectName("arrival_date_line_2")
        self.verticalLayout.addWidget(self.arrival_date_line_2)
        self.label_7 = QtWidgets.QLabel(Dialog)
        self.label_7.setStyleSheet("color:white;")
        self.label_7.setObjectName("label_7")
        self.verticalLayout.addWidget(self.label_7)
        self.section_id_line = QtWidgets.QLineEdit(Dialog)
        self.section_id_line.setObjectName("section_id_line")
        self.verticalLayout.addWidget(self.section_id_line)
        self.insert_btn = QtWidgets.QPushButton(Dialog)
        self.insert_btn.setStyleSheet("background-color: white;\n"
"color : black;\n"
"")
        self.insert_btn.setObjectName("insert_btn")
        self.verticalLayout.addWidget(self.insert_btn)

        self.retranslateUi(Dialog)
        QtCore.QMetaObject.connectSlotsByName(Dialog)

    def retranslateUi(self, Dialog):
        _translate = QtCore.QCoreApplication.translate
        Dialog.setWindowTitle(_translate("Dialog", "Dialog"))
        self.label.setText(_translate("Dialog", "PRINTERS"))
        self.macAddress.setText(_translate("Dialog", "Mac Address"))
        self.label_3.setText(_translate("Dialog", "Brand"))
        self.label_4.setText(_translate("Dialog", "Model"))
        self.label_5.setText(_translate("Dialog", "Ink Storage"))
        self.label_6.setText(_translate("Dialog", "Arrival Date"))
        self.label_7.setText(_translate("Dialog", "Section ID"))
        self.insert_btn.setText(_translate("Dialog", "Insert"))
